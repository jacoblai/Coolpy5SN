...
func NewTGateway(udpport int,broker string, stopsig chan os.Signal) *TGateway {
	t := &TGateway{
		stopsig,
		udpport,
		broker,
...
func (t *TGateway) Start() {
	go t.awaitStop()
	INFO.Println("Gataway is started on",t.port)
	INFO.Println("Broker Addr",t.mqttBroker)
	listen(t)
}

func (t *TGateway) awaitStop() {
	<-t.stopsig
	INFO.Println("Gateway is stopped")
	os.Exit(0)
}
